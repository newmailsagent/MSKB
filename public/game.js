  ...
  _registerHandlers() {
    ...
    this.socket.on('reconnect_ok', ({ roomId, started, isMyTurn, turnSecondsLeft }) => {
      ...
      if (started) {
        ...
        // Восстанавливаем нормальный заголовок хода
        updateGameStatus();
      }
    });
...
// Статический справочник — гарантирует русские названия до загрузки магазина
const TITLE_NAMES_RU = {
  title_fleet_recruit:   'Новобранец флота',
  title_exp_tester:      'Опытный испытатель',
  title_admiral:         'Адмирал',
  title_marshal:         'Маршал',
  title_real_intel:      'Реальный интеллект',
  title_star_scout:      'Звёздный разведчик',
  title_anon_hunter:     'Анонимный охотник',
  title_friendly_fleet:  'Дружеский флот',
  title_captain_tester:  'Капитан-испытатель',
  title_duelist:         'Дуэлянт',
  title_sea_strategist:  'Морской стратег',
  title_first_time:      'Время первых',
  title_determined:      'Целеустремлённый',
  title_collector:       'Коллекционер',
  title_recruiter:       'Рекрутер',
  title_space_navigator: 'Космический навигатор',
  title_engineer:        'Инженер',
  title_cyber_pirate:    'Кибер-пират',
  title_patron:          'Меценат',
  title_davy_jones:      'Дейви Джонс',
  title_four_deck:       'Четырёхпалубный',
  title_commander:       'Главнокомандующий',
  title_default:         'По умолчанию',
};
...
async function applyTitleAndRefresh(itemId, equip = true) {
  ...
  showLoaderOverMenu(hide => {
    setTimeout(() => {
      hide && hide();
      // Обновляем везде: меню, профиль, игровое поле
      updateMenuLevel();
      renderInventory();
      renderShopGrid();
      // Профиль — только если открыт
      if (currentScreen === 'profile') {
        const prog = getXpProgress(App.user.xp || 0);
        const profileRankEl = document.getElementById('profile-rank');
        if (profileRankEl) {
          const tid = _shopEquipped['title'];
          if (tid && tid !== 'title_default') {
            const tItem = _shopItems.find(i => i.id === tid);
            if (tItem) {
              profileRankEl.innerHTML = `<span style="color:${titleRankColor(tItem.title_rank)};font-weight:700">${tItem.name}</span>`;
            } else { profileRankEl.textContent = prog.rank; }
          } else { profileRankEl.textContent = prog.rank; }
        }
        showScreen('profile');
      } else {
...
async function renderInventory() {
  ...
  grid.querySelectorAll('[data-inv-action]').forEach(btn => {
    ...
    btn.addEventListener('click', async e => {
      ...
      if (item.type === 'title') {
        // Снять "По умолчанию" = снять текущее звание
        if (itemId === 'title_default') {
          await applyTitleAndRefresh(null, false);
        } else {
          await applyTitleAndRefresh(itemId, true);
        }
        return;
